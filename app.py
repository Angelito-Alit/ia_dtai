    ...
    if intent == 'saludo':
        ...
        if data:
            ...
            for row in data:
                ...
                materias_por_carrera[carrera].append(row)
            
            total_evaluaciones = sum([row['total_evaluaciones'] for row in data if row['total_evaluaciones']])
            
            for carrera, materias in materias_por_carrera.items():
                response += f"**{carrera.upper()}**\n"
                materias_ordenadas = sorted(materias, key=lambda x: x['cuatrimestre'])
                
                for materia in materias_ordenadas:
                    if materia['total_evaluaciones'] and materia['total_evaluaciones'] > 0:
                        tasa_aprobacion = (materia['aprobados'] / materia['total_evaluaciones'] * 100)
                        tasa_excelencia = (materia['excelentes'] / materia['total_evaluaciones'] * 100)
                        
                        nivel_dificultad = "Alta Dificultad" if tasa_aprobacion < 70 else "Dificultad Media" if tasa_aprobacion < 85 else "Accesible"
                        
                        response += f"• **{materia['nombre']}** ({materia['codigo']})\n"
                        response += f"  Cuatrimestre: {materia['cuatrimestre']} | Créditos: {materia['creditos']}\n"
                        response += f"  Promedio General: {materia['promedio_materia']} | {nivel_dificultad}\n"
                        response += f"  Evaluaciones: {materia['total_evaluaciones']} | Aprobación: {tasa_aprobacion:.1f}%\n"
                        response += f"  Excelencia: {materia['excelentes']} estudiantes ({tasa_excelencia:.1f}%)\n"
                        response += f"  En Riesgo: {materia['en_riesgo']} estudiantes\n"
                        if materia['descripcion']:
                            response += f"  Descripción: {materia['descripcion'][:100]}...\n"
                        response += "\n"
                    else:
                        response += f"• **{materia['nombre']}** ({materia['codigo']})\n"
                        response += f"  Cuatrimestre: {materia['cuatrimestre']} | Créditos: {materia['creditos']}\n"
                        response += f"  Estado: Sin evaluaciones registradas\n\n"
            
            # Estadísticas generales
            materias_con_datos = [row for row in data if row['total_evaluaciones'] and row['total_evaluaciones'] > 0]
            
            if materias_con_datos:
                promedio_sistema = sum([row['promedio_materia'] * row['total_evaluaciones'] for row in materias_con_datos]) / sum([row['total_evaluaciones'] for row in materias_con_datos])
                total_aprobados = sum([row['aprobados'] for row in materias_con_datos])
                total_reprobados = sum([row['reprobados'] for row in materias_con_datos])
                total_excelentes = sum([row['excelentes'] for row in materias_con_datos])
                
                response += "**Estadísticas Generales del Sistema:**\n"
                response += f"• Total de asignaturas activas: {len(data)}\n"
                response += f"• Asignaturas con evaluaciones: {len(materias_con_datos)}\n"
                response += f"• Promedio general del sistema: {promedio_sistema:.2f}\n"
                response += f"• Total de evaluaciones procesadas: {total_evaluaciones}\n"
                response += f"• Tasa global de aprobación: {(total_aprobados/total_evaluaciones*100):.1f}%\n"
                response += f"• Estudiantes en excelencia: {total_excelentes} ({(total_excelentes/total_evaluaciones*100):.1f}%)\n"
                
                # Análisis de materias problemáticas
                materias_problematicas = [row for row in materias_con_datos if (row['reprobados'] / row['total_evaluaciones'] * 100) > 30]
                materias_destacadas = [row for row in materias_con_datos if (row['excelentes'] / row['total_evaluaciones'] * 100) > 40]
                
                if materias_problematicas:
                    response += f"\n**Materias que Requieren Atención Especial:**\n"
                    for materia in materias_problematicas[:5]:
                        tasa_reprobacion = (materia['reprobados'] / materia['total_evaluaciones'] * 100)
                        response += f"• {materia['nombre']} - {tasa_reprobacion:.1f}% de reprobación\n"
                
                if materias_destacadas:
                    response += f"\n**Materias Modelo de Excelencia:**\n"
                    for materia in materias_destacadas[:5]:
                        tasa_excelencia = (materia['excelentes'] / materia['total_evaluaciones'] * 100)
                        response += f"• {materia['nombre']} - {tasa_excelencia:.1f}% de estudiantes en excelencia\n"
            
            response += "\n¿Te interesa que analice el desempeño específico de alguna materia, genere estrategias de mejoramiento para materias problemáticas, o prefieres un análisis comparativo del rendimiento por cuatrimestre?"
            
            return response
        else:
            return "No encontré asignaturas activas en el sistema actualmente. Esto podría indicar que estamos en un período de actualización del catálogo académico o que las materias para el próximo período aún no han sido activadas. Te recomiendo contactar con la coordinación académica para verificar el estado del catálogo de materias. Si necesitas información sobre asignaturas de períodos anteriores, análisis históricos de rendimiento por materia, o ayuda con la configuración de nuevas asignaturas, puedo asistirte una vez que los datos estén disponibles."
    
    elif intent == 'consulta_profesores':
        query = """
        SELECT u.nombre, u.apellido, p.numero_empleado, p.activo, p.fecha_ingreso,
               p.experiencia_años, p.es_tutor, COUNT(DISTINCT pag.grupo_id) as grupos_asignados,
               COUNT(DISTINCT pag.asignatura_id) as materias_impartidas,
               car.nombre as carrera_principal, u.correo, u.telefono
        FROM profesores p
        JOIN usuarios u ON p.usuario_id = u.id
        LEFT JOIN profesor_asignatura_grupo pag ON p.id = pag.profesor_id AND pag.activo = 1
        LEFT JOIN grupos g ON pag.grupo_id = g.id
        LEFT JOIN carreras car ON g.carrera_id = car.id
        WHERE p.activo = 1
        GROUP BY p.id, u.nombre, u.apellido, p.numero_empleado, p.activo, p.fecha_ingreso,
                 p.experiencia_años, p.es_tutor, car.nombre, u.correo, u.telefono
        ...
        if data:
            response = "**Directorio Completo del Personal Docente**\n\n"
            response += f"He procesado la información completa del personal docente activo. Tenemos {len(data)} profesores registrados y activos en el sistema. Aquí está el análisis detallado:\n\n"
            
            profesores_con_carga = [row for row in data if row['grupos_asignados'] > 0]
            profesores_sin_carga = [row for row in data if row['grupos_asignados'] == 0]
            total_grupos = sum([row['grupos_asignados'] for row in data])
            total_materias = sum([row['materias_impartidas'] for row in data])
            
            response += "**Personal Docente con Carga Académica:**\n"
            for i, profesor in enumerate(profesores_con_carga, 1):
                años_servicio = (datetime.now() - profesor['fecha_ingreso']).days // 365 if profesor['fecha_ingreso'] else 0
                tutor_status = "Tutor Académico" if profesor['es_tutor'] else "Docente"
                
                response += f"{i}. **{profesor['nombre']} {profesor['apellido']}** - {tutor_status}\n"
                response += f"   No. Empleado: {profesor['numero_empleado']}\n"
                response += f"   Experiencia: {profesor['experiencia_años']} años | Antigüedad: {años_servicio} años\n"
                response += f"   Carga Actual: {profesor['grupos_asignados']} grupos, {profesor['materias_impartidas']} materias\n"
                if profesor['carrera_principal']:
                    response += f"   Área Principal: {profesor['carrera_principal']}\n"
                response += f"   Contacto: {profesor['correo']}"
                if profesor['telefono']:
                    response += f" | Tel: {profesor['telefono']}"
                response += "\n\n"
            
            if profesores_sin_carga:
                response += f"**Personal Docente Disponible ({len(profesores_sin_carga)} profesores):**\n"
                for profesor in profesores_sin_carga:
                    response += f"• {profesor['nombre']} {profesor['apellido']} - {profesor['numero_empleado']}\n"
                    response += f"  Experiencia: {profesor['experiencia_años']} años | Disponible para asignación\n\n"
            
            # Estadísticas del personal docente
            experiencia_promedio = sum([row['experiencia_años'] for row in data if row['experiencia_años']]) / len([row for row in data if row['experiencia_años']])
            tutores_activos = len([row for row in data if row['es_tutor']])
            
            response += "**Estadísticas del Personal Docente:**\n"
            response += f"• Total de profesores activos: {len(data)}\n"
            response += f"• Profesores con carga académica: {len(profesores_con_carga)} ({(len(profesores_con_carga)/len(data)*100):.1f}%)\n"
            response += f"• Profesores disponibles: {len(profesores_sin_carga)}\n"
            response += f"• Tutores académicos activos: {tutores_activos}\n"
            response += f"• Experiencia promedio: {experiencia_promedio:.1f} años\n"
            response += f"• Total de grupos atendidos: {total_grupos}\n"
            response += f"• Total de materias impartidas: {total_materias}\n"
            
            # Análisis de carga de trabajo
            if profesores_con_carga:
                carga_promedio = sum([row['grupos_asignados'] for row in profesores_con_carga]) / len(profesores_con_carga)
                profesores_sobrecargados = [row for row in profesores_con_carga if row['grupos_asignados'] > carga_promedio * 1.5]
                profesores_subcargados = [row for row in profesores_con_carga if row['grupos_asignados'] < carga_promedio * 0.5]
                
                response += f"• Carga promedio por profesor: {carga_promedio:.1f} grupos\n"
                
                if profesores_sobrecargados:
                    response += f"\n**Profesores con Carga Alta ({len(profesores_sobrecargados)}):**\n"
                    for prof in profesores_sobrecargados:
                        response += f"• {prof['nombre']} {prof['apellido']}: {prof['grupos_asignados']} grupos\n"
                
                if profesores_subcargados:
                    response += f"\n**Profesores con Capacidad Disponible ({len(profesores_subcargados)}):**\n"
                    for prof in profesores_subcargados:
                        response += f"• {prof['nombre']} {prof['apellido']}: {prof['grupos_asignados']} grupos\n"
            
            # Análisis por área académica
            areas_academicas = {}
            for row in profesores_con_carga:
                if row['carrera_principal']:
                    area = row['carrera_principal']
                    if area not in areas_academicas:
                        areas_academicas[area] = []
                    areas_academicas[area].append(row)
            
            if areas_academicas:
                response += f"\n**Distribución por Área Académica:**\n"
                for area, profesores in areas_academicas.items():
                    grupos_area = sum([prof['grupos_asignados'] for prof in profesores])
                    response += f"• {area}: {len(profesores)} profesores, {grupos_area} grupos\n"
            
            response += "\n¿Te interesa que analice la carga de trabajo específica de algún profesor, genere reportes de disponibilidad para nuevas asignaciones, o prefieres información detallada sobre algún área académica específica?"
            
            return response
        else:
            return "No encontré profesores activos registrados en el sistema actualmente. Esto podría indicar un problema con la sincronización de datos o que estamos en un período de actualización del personal docente. Te recomiendo verificar con recursos humanos sobre el estado del registro de profesores. Si necesitas información sobre profesores de períodos anteriores, análisis históricos de carga académica, o ayuda con el registro de nuevo personal docente, puedo asistirte una vez que los datos estén disponibles."
    
    elif intent == 'consulta_estudiantes':
        query = """
        SELECT u.nombre, u.apellido, al.matricula, al.promedio_general, al.estado_alumno,
               al.fecha_ingreso, car.nombre as carrera, al.cuatrimestre_actual, al.creditos_acumulados,
        ...
        GROUP BY al.id, u.nombre, u.apellido, al.matricula, al.promedio_general, 
                 al.estado_alumno, al.fecha_ingreso, car.nombre, al.cuatrimestre_actual,
                 al.creditos_acumulados, u.correo, u.telefono
        ...
        if data:
            response = "**Directorio Completo de Estudiantes Activos**\n\n"
            response += f"He procesado la información completa de estudiantes activos en el sistema. Tenemos {len(data)} estudiantes registrados y activos. Aquí está el análisis detallado:\n\n"
            
            # Estudiantes destacados (top 10)
            estudiantes_destacados = data[:10]
            response += "**Estudiantes con Mejor Rendimiento Académico:**\n"
            for i, estudiante in enumerate(estudiantes_destacados, 1):
                tiempo_estudio = (datetime.now() - estudiante['fecha_ingreso']).days // 30 if estudiante['fecha_ingreso'] else 0
                eficiencia = (estudiante['materias_aprobadas'] / estudiante['materias_cursadas'] * 100) if estudiante['materias_cursadas'] > 0 else 0
                
                response += f"{i}. **{estudiante['nombre']} {estudiante['apellido']}** (Matrícula: {estudiante['matricula']})\n"
                response += f"   Programa: {estudiante['carrera']} | Cuatrimestre: {estudiante['cuatrimestre_actual']}\n"
                response += f"   Promedio General: {estudiante['promedio_general']:.2f}\n"
                response += f"   Progreso Académico: {estudiante['materias_aprobadas']}/{estudiante['materias_cursadas']} materias ({eficiencia:.1f}% eficiencia)\n"
                response += f"   Créditos Acumulados: {estudiante['creditos_acumulados']}\n"
                response += f"   Tiempo en el Programa: {tiempo_estudio} meses\n"
                response += f"   Contacto: {estudiante['correo']}"
                if estudiante['telefono']:
                    response += f" | Tel: {estudiante['telefono']}"
                response += "\n\n"
            ...
            for estudiante in data:
                ...
                estudiantes_por_carrera[carrera].append(estudiante)
            
            response += "**Distribución por Programa Académico:**\n"
            for carrera, estudiantes in estudiantes_por_carrera.items():
                promedio_carrera = sum([est['promedio_general'] for est in estudiantes if est['promedio_general']]) / len([est for est in estudiantes if est['promedio_general']])
                estudiantes_excelencia = len([est for est in estudiantes if est['promedio_general'] >= 9.0])
                estudiantes_riesgo = len([est for est in estudiantes if est['promedio_general'] < 7.0])
                
                response += f"• **{carrera}**: {len(estudiantes)} estudiantes\n"
                response += f"  Promedio del programa: {promedio_carrera:.2f}\n"
                response += f"  En excelencia: {estudiantes_excelencia} ({(estudiantes_excelencia/len(estudiantes)*100):.1f}%)\n"
                response += f"  En riesgo: {estudiantes_riesgo} ({(estudiantes_riesgo/len(estudiantes)*100):.1f}%)\n\n"
            
            # Estadísticas generales
            promedio_general_sistema = sum([est['promedio_general'] for est in data if est['promedio_general']]) / len([est for est in data if est['promedio_general']])
            total_materias_cursadas = sum([est['materias_cursadas'] for est in data])
            total_materias_aprobadas = sum([est['materias_aprobadas'] for est in data])
            total_creditos = sum([est['creditos_acumulados'] for est in data if est['creditos_acumulados']])
            
            response += "**Estadísticas Generales del Estudiantado:**\n"
            response += f"• Total de estudiantes activos: {len(data)}\n"
            response += f"• Promedio general del sistema: {promedio_general_sistema:.2f}\n"
            response += f"• Total de materias cursadas: {total_materias_cursadas}\n"
            response += f"• Tasa global de aprobación: {(total_materias_aprobadas/total_materias_cursadas*100):.1f}%\n"
            response += f"• Total de créditos acumulados: {total_creditos}\n"
            response += f"• Promedio de créditos por estudiante: {(total_creditos/len(data)):.1f}\n"
            
            # Análisis por cuatrimestre
            distribucion_cuatrimestre = {}
            for estudiante in data:
                cuatri = estudiante['cuatrimestre_actual']
                distribucion_cuatrimestre[cuatri] = distribucion_cuatrimestre.get(cuatri, 0) + 1
            
            response += "\n**Distribución por Cuatrimestre:**\n"
            for cuatri in sorted(distribucion_cuatrimestre.keys()):
                cantidad = distribucion_cuatrimestre[cuatri]
                response += f"• Cuatrimestre {cuatri}: {cantidad} estudiantes ({(cantidad/len(data)*100):.1f}%)\n"
            
            # Estudiantes que requieren atención
            estudiantes_riesgo_total = [est for est in data if est['promedio_general'] < 7.0]
            estudiantes_excelencia_total = [est for est in data if est['promedio_general'] >= 9.0]
            
            if estudiantes_riesgo_total:
                response += f"\n**Estudiantes que Requieren Seguimiento ({len(estudiantes_riesgo_total)}):**\n"
                for est in estudiantes_riesgo_total[:5]:
                    response += f"• {est['nombre']} {est['apellido']} - Promedio: {est['promedio_general']:.2f} ({est['carrera']})\n"
            
            if estudiantes_excelencia_total:
                response += f"\n**Estudiantes en Excelencia Académica ({len(estudiantes_excelencia_total)}):**\n"
                for est in estudiantes_excelencia_total[:5]:
                    response += f"• {est['nombre']} {est['apellido']} - Promedio: {est['promedio_general']:.2f} ({est['carrera']})\n"
            
            response += "\n¿Te interesa que analice el progreso específico de algún estudiante, genere reportes de seguimiento académico, o prefieres información detallada sobre algún programa académico específico?"
            
            return response
        else:
            return "No encontré estudiantes activos registrados en el sistema actualmente. Esto podría indicar que estamos en un período de transición entre cuatrimestres, que los estudiantes aún no han completado su proceso de inscripción, o que hay un problema con la sincronización de datos. Te recomiendo verificar con el área de servicios escolares sobre el estado de las inscripciones. Si necesitas información sobre estudiantes de períodos anteriores, análisis históricos de rendimiento, o ayuda con procesos de inscripción, puedo asistirte una vez que los datos estén disponibles."
    
    elif intent == 'calificaciones':
        return get_calificaciones_response()
    
    elif intent == 'riesgo':
        return get_riesgo_response()
    
    elif intent == 'promedio':
        return get_promedio_response()
    
    elif intent == 'estadisticas':
        return get_estadisticas_response()
    
    elif intent == 'agradecimiento':
        agradecimientos = [
            "De nada, es un placer ayudarte con el análisis de datos académicos. Mi función es hacer que la información compleja sea fácil de entender y útil para la toma de decisiones. Siempre estoy disponible para cualquier consulta sobre estadísticas, rendimiento estudiantil, análisis de profesores, horarios, materias, casos de riesgo, o cualquier aspecto del sistema educativo que necesites analizar. ¿Hay algo más en lo que pueda asistirte?",
            "Un placer total poder contribuir con información valiosa para la gestión académica. Cada consulta me permite demostrar el valor de los datos para mejorar la educación. Recuerda que tengo acceso completo a toda la base de datos institucional y puedo generar análisis detallados, reportes personalizados, estadísticas actualizadas y insights estratégicos en cualquier momento. ¿Necesitas algún otro análisis?",
            "Para eso estoy aquí, siempre es gratificante poder ayudar con análisis educativos que generen valor real. Mi objetivo es convertir datos complejos en información accionable que apoye las mejores decisiones académicas. Tienes acceso permanente a análisis de estudiantes, profesores, horarios, materias, estadísticas institucionales y cualquier métrica que requieras. ¿Qué más podemos analizar juntos?",
            "No hay de qué, me satisface enormemente poder asistir en la mejora continua del sistema educativo mediante análisis de datos inteligentes. Cada insight que generamos juntos contribuye a una mejor experiencia académica. Estoy completamente disponible para futuras consultas sobre cualquier aspecto de la gestión educativa, desde métricas de rendimiento hasta análisis predictivos. ¿Hay algún otro tema que te interese explorar?"
        ]
        return random.choice(agradecimientos)
    
    elif intent == 'afirmacion':
        afirmaciones = [
            "Perfecto, me alegra que la información sea útil. Como tu asistente especializado en datos académicos, estoy aquí para profundizar en cualquier aspecto que necesites. Puedo generar análisis más detallados, comparaciones históricas, proyecciones futuras, o explorar correlaciones entre diferentes métricas del sistema educativo. ¿En qué aspectos específicos te gustaría que profundice?",
            "Excelente, es genial saber que vamos por el camino correcto con el análisis. Tengo capacidad para expandir cualquier tema que hayamos tocado, generar reportes complementarios, crear visualizaciones de datos, o analizar tendencias específicas que te interesen. Mi base de datos está actualizada y lista para cualquier consulta adicional. ¿Qué más te gustaría explorar?",
            "Fantástico, me complace que el análisis sea de valor. Como sistema inteligente de análisis educativo, puedo continuar desarrollando insights más profundos, generar recomendaciones estratégicas específicas, o ayudarte con análisis comparativos y predictivos. Todas las métricas académicas están a mi disposición para brindarte la información más relevante. ¿Hay algún área específica que requiera más atención?"
        ]
        return random.choice(afirmaciones)
    
    elif intent == 'negacion':
        negaciones = [
            "Entiendo perfectamente, no hay problema. Como tu asistente de análisis académico, estoy aquí para adaptarme a tus necesidades específicas. Si prefieres explorar un tema diferente, cambiar el enfoque del análisis, o necesitas información sobre algún otro aspecto del sistema educativo, solo dímelo. Tengo acceso completo a todas las métricas: estudiantes, profesores, horarios, materias, estadísticas institucionales y mucho más. ¿Hay algo diferente en lo que pueda ayudarte?",
            "Sin ningún problema, comprendo que quizás necesites un enfoque diferente o información sobre otro tema. Mi especialidad es adaptarme a lo que realmente necesitas para la gestión académica efectiva. Puedo ayudarte con análisis de rendimiento, reportes de profesores, gestión de horarios, evaluación de materias, seguimiento de estudiantes, o cualquier otra consulta educativa que tengas en mente. ¿Qué te gustaría explorar?",
            "Está perfecto, respeto tu decisión. Como sistema de análisis educativo, mi objetivo es proporcionarte exactamente la información que necesitas, cuando la necesitas. Si hay algún otro aspecto de la gestión académica que te interese, alguna métrica específica que quieras revisar, o un análisis diferente que consideres más útil, estoy completamente disponible para ayudarte. ¿En qué más puedo asistirte?"
        ]
        return random.choice(negaciones)
    
    else:
        # Respuesta inteligente para casos no clasificados
        respuestas_generales = [
            f"Entiendo tu consulta sobre '{message}'. Como especialista en análisis de datos educativos, puedo ayudarte con una amplia gama de información académica. Tengo acceso a estadísticas de estudiantes, análisis de profesores, información de horarios y materias, casos de riesgo académico, rendimiento por programas, métricas institucionales y mucho más. Para brindarte la información más precisa y útil, ¿podrías especificar qué tipo de análisis o datos específicos te interesan?",
            f"Interesante punto sobre '{message}'. Mi sistema está diseñado para procesar y analizar cualquier aspecto del ecosistema educativo de DTAI. Puedo generar reportes sobre rendimiento académico, análisis de tendencias, información detallada de estudiantes y profesores, gestión de horarios, evaluación de materias, seguimiento de casos de riesgo, estadísticas institucionales y análisis predictivos. ¿Hay algún área específica en la que te gustaría que me enfoque?",
            f"Comprendo tu interés en '{message}'. Como tu asistente inteligente de análisis académico, tengo la capacidad de explorar prácticamente cualquier dimensión de los datos educativos. Desde análisis individuales de estudiantes hasta métricas institucionales complejas, pasando por evaluación de profesores, optimización de horarios, rendimiento por materias y proyecciones futuras. ¿Qué aspecto específico te gustaría que analice en profundidad?"
        ]
        return random.choice(respuestas_generales)

def get_calificaciones_response():
    query = """
    ...
    if data:
        response = "**Sistema de Evaluación Académica - Análisis Completo de Calificaciones**\n\n"
        response += f"He procesado las evaluaciones más recientes del sistema académico. Los datos muestran {len(data)} evaluaciones con calificaciones registradas. Aquí está el análisis detallado del rendimiento académico:\n\n"
        
        # Análisis estadístico
        calificaciones = [row['calificacion_final'] for row in data]
